...
int main() {
    ...
    uint32_t totalWords;
    // Read the header (Total Words)
    if (!inFile.read((char*)&totalWords, sizeof(totalWords))) {
        cerr << "Error reading header." << endl;
        return 1;
    }
    
    cout << "Total Words to process: " << totalWords << endl;

    // 2. Loop through the Monolith (Dense Format: Count, Postings...)
    for (uint32_t wordID = 0; wordID < totalWords; ++wordID) {
        
        uint32_t count;
        // Read the count (Document Frequency)
        if (!inFile.read((char*)&count, sizeof(count))) {
            cerr << "Unexpected end of file at WordID " << wordID << endl;
            break;
        }
        ...
        // Barrels store explicit WordID because they are splits
        ...
        if (count > 0) {
            ...
            // Read from Source
            if (!inFile.read(buffer.data(), bytesToCopy)) {
                cerr << "Error reading postings for WordID " << wordID << endl;
                break;
            }
